...
interval IntervalArithmetic::Inverse(const interval& x) {
	...
	z2.a = 1 / x.a;
	fesetround(FE_TONEAREST);
	return (DIntWidth(z1) >= DIntWidth(z2)) ? z1 : z2;
}
	...
	if (x.a > x.b)
		...
		do {
			...
			if (is_even)
				...
				w1 = IAdd(w, s);
			if ((w.a != 0) && (w.b != 0))
				if ((std::abs(w.a - w1.a) / std::abs(w.a) < 1e-18) && (std::abs(w.b - w1.b)
						/ std::abs(w.b) < 1e-18))
					finished = true;
				else
					;
			else if ((w.a == 0) && (w.b != 0))
				if ((std::abs(w.a - w1.a) < 1e-18) && (std::abs(w.b - w1.b) / std::abs(w.b)
						< 1e-18))
					finished = true;
				else
					;
				...
				if ((std::abs(w.a - w1.a) / std::abs(w.a) < 1e-18) & (std::abs(w.b - w1.b)
						< 1e-18))
					finished = true;
				else if ((std::abs(w.a - w1.a) < 1e-18) & (std::abs(w.b - w1.b) < 1e-18))
					finished = true;
...
interval IntervalArithmetic::IExp(const interval& x, int & st) {
	...
	if (x.a > x.b)
		...
		do {
			...
			e = IMul(e, IDiv(x, d));
			w1 = IAdd(w, e);
			if ((std::abs(w.a - w1.a) / std::abs(w.a) < 1e-18) && (std::abs(w.b - w1.b) / std::abs(
					w.b) < 1e-18)) {
				finished = true;
				return w1;
			}
...
interval IntervalArithmetic::ISqr(const interval& x, int & st) {
	...
	if (x.a > x.b)
		...
		if ((x.a <= 0) && (x.b >= 0))
			...
			minx = x.b;
		maxx = (std::abs(x.a) > std::abs(x.b)) ? std::abs(x.a) : std::abs(x.b);
		fesetround(FE_DOWNWARD);
		r.a = minx * minx;
		fesetround(FE_UPWARD);
		r.b = maxx * maxx;
